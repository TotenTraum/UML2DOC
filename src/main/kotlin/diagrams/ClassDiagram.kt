...
class ClassDiagram(private val stream: Stream<Quark<Entity>>) : IDiagram {
    ...
    fun fieldTable(factory: IDocObjectFactory, entity: Quark<Entity>): ITable {
        ...
        entity.data.bodier.fieldsToDisplay.forEachIndexed { index, charSequence ->
            val list = charSequence.toString().trim().replace("\\s+".toRegex(), " ").replace(" :".toRegex(), ":").split(": ", ":", " ")
            if (list.count() != 2)
    ...
    fun ITable.setText(factory: IDocObjectFactory, row: Int, col: Int, text: String, fontSize: Int = 24) {
    ...
    fun methodTable(factory: IDocObjectFactory, entity: Quark<Entity>): ITable {
        ...
        entity.data.bodier.methodsToDisplay.forEachIndexed { index, charSequence ->
            ...
            table.setText(factory, index + 1, 0,"${list[0].visibilityStatus()} ${list[1]}")
            table.setText(factory, index + 1, 1, "")
        }
        return table
    }

    fun String.visibilityStatus(): String{
        return when(trim()){
            "+" -> "public"
            "#" -> "protected"
            "~" -> "package private"
            "-" -> "private"
            else -> TODO("add special exception")
        }
    }
}